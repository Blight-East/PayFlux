    ...
    return (
        <div className="border border-slate-800 rounded-lg px-5 py-4">
            ...
            <div className="grid grid-cols-4 gap-4 mb-3">
                <div>
                    <span className="text-[10px] text-slate-500 uppercase tracking-wider block mb-1">Tier Accuracy</span>
                    <span className="text-lg font-mono font-bold text-slate-200">{data.tierAccuracy}%</span>
                </div>
                <div>
                    <span className="text-[10px] text-slate-500 uppercase tracking-wider block mb-1">Trend Accuracy</span>
                    <span className="text-lg font-mono font-bold text-slate-200">{data.trendAccuracy}%</span>
                </div>
                <div>
                    <span className="text-[10px] text-slate-500 uppercase tracking-wider block mb-1">Mean Variance</span>
                    <span className="text-lg font-mono font-bold text-slate-200">±{data.meanVarianceBps} bps</span>
                </div>
                <div>
                    <span className="text-[10px] text-slate-500 uppercase tracking-wider block mb-1">Evaluations</span>
                    <span className="text-lg font-mono font-bold text-slate-200">{data.totalEvaluations}</span>
                </div>
            </div>

            {/* Secondary metrics */}
            <div className="border-t border-slate-800 pt-2 flex items-center space-x-6">
