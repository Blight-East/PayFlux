...
// Pinned to globalThis so the store survives module re-evaluation during
// Next.js dev-mode HMR — otherwise a state generated by `/api/stripe/authorize`
// can be lost before `/api/stripe/callback` re-imports this module.
const GLOBAL_MEMORY_STORE_KEY = Symbol.for('payflux.oauth-state.memoryStore');
type GlobalWithMemoryStore = typeof globalThis & {
    [GLOBAL_MEMORY_STORE_KEY]?: Map<string, OAuthState>;
};
const globalMemory = globalThis as GlobalWithMemoryStore;
const memoryStore =
    globalMemory[GLOBAL_MEMORY_STORE_KEY] ??
    (globalMemory[GLOBAL_MEMORY_STORE_KEY] = new Map<string, OAuthState>());
