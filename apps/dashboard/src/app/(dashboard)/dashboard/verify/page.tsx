    ...
    return (
        <div className="p-8 max-w-4xl">
            ...
            <section className="mb-8">
                <div className="text-[10px] text-slate-500 uppercase tracking-[0.2em] font-bold mb-4">Signed Artifact</div>
                <textarea
                    value={input}
                    onChange={(e) => setInput(e.target.value)}
                    placeholder="Paste JSON export (Board Reserve Report or Evidence Export envelope)"
                    className="w-full h-64 bg-slate-950 border border-slate-800 rounded-xl px-4 py-3 text-xs font-mono text-slate-300 resize-none focus:outline-none focus:border-slate-600 transition-colors placeholder-slate-700"
                />
            ...
            {result && (
                <section className="mb-16">
                    <div className="text-[10px] text-slate-500 uppercase tracking-[0.2em] font-bold mb-4">Verification Result</div>
                    ...
                    {result.error ? (
                        ...
                        <div className="border border-slate-800 rounded-xl px-6 py-6 space-y-6">
                            {/* Overall Status */}
                            ...
                            <div className="grid grid-cols-3 gap-4">
                                <div>
                                    ...
                                    <div className="text-xs font-mono text-slate-300">{result.reportType}</div>
                                </div>
                                <div>
                                    <div className="text-[10px] text-slate-500 uppercase tracking-wider mb-1">Version</div>
                                    <div className="text-xs font-mono text-slate-300">{result.reportVersion}</div>
                                </div>
                                <div>
                                    <div className="text-[10px] text-slate-500 uppercase tracking-wider mb-1">Generated</div>
                            ...
                            {/* Hash Comparison */}
                            <div className="bg-slate-900/30 border border-slate-800/60 rounded-lg px-4 py-3 space-y-2">
                                <div className="text-[10px] text-slate-500 uppercase tracking-wider mb-2">Hash Verification</div>
                                <div className="flex items-center justify-between">
                                    <span className="text-[10px] text-slate-500">Declared</span>
                                    <span className="text-[10px] font-mono text-slate-400">{result.artifactHash ? `${result.artifactHash.slice(0, 32)}…` : '—'}</span>
                                </div>
                                <div className="flex items-center justify-between">
                                    <span className="text-[10px] text-slate-500">Recomputed</span>
                                    <span className="text-[10px] font-mono text-slate-400">{result.recomputedHash ? `${result.recomputedHash.slice(0, 32)}…` : '—'}</span>
                                </div>
                            ...
                            {result.ledgerRecords > 0 && (
                                <div className="bg-slate-900/30 border border-slate-800/60 rounded-lg px-4 py-3 space-y-2">
                                    <div className="text-[10px] text-slate-500 uppercase tracking-wider mb-2">Ledger Records</div>
                                    <div className="flex items-center justify-between">
                                        <span className="text-[10px] text-slate-500">Total Records</span>
                                        <span className="text-xs font-mono text-slate-300">{result.ledgerRecords}</span>
                                    </div>
                                    <div className="flex items-center justify-between">
                                        <span className="text-[10px] text-slate-500">Valid Hash</span>
                                        <span className="text-xs font-mono text-emerald-500">{result.recordsWithValidHash} / {result.ledgerRecords}</span>
                                    </div>
                                    <div className="flex items-center justify-between">
                                        <span className="text-[10px] text-slate-500">Valid Signature</span>
            ...
            {/* Methodology */}
            <section className="mb-16">
                <div className="text-[10px] text-slate-500 uppercase tracking-[0.2em] font-bold mb-4">Verification Methodology</div>
                <div className="border border-slate-800/60 rounded-xl px-6 py-5 space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <div className="text-[10px] font-mono font-bold text-slate-300 mb-1">Hash Algorithm</div>
                            <div className="text-[10px] text-slate-500">SHA-256</div>
                        </div>
                        <div>
                            <div className="text-[10px] font-mono font-bold text-slate-300 mb-1">Signature Algorithm</div>
                            <div className="text-[10px] text-slate-500">HMAC-SHA256</div>
                        </div>
                        <div>
                            <div className="text-[10px] font-mono font-bold text-slate-300 mb-1">Client-Side Verification</div>
                            <div className="text-[10px] text-slate-500">Hash only (no secret access)</div>
                        </div>
                        <div>
                            <div className="text-[10px] font-mono font-bold text-slate-300 mb-1">Server-Side Verification</div>
                            <div className="text-[10px] text-slate-500">Hash + HMAC signature (requires EVIDENCE_SECRET)</div>
                        </div>
                    </div>
                    <div className="text-[10px] text-slate-600 font-mono pt-2 border-t border-slate-800/40">
