...
export default function TrustDashboardPlayground() {
    ...
    return (
        <div className="h-screen w-screen flex flex-col overflow-hidden bg-background text-slate-400 font-sans relative">
            ...
            <div className="absolute top-20 right-6 z-[60] bg-surface p-4 border border-border shadow-2xl mono text-[10px] space-y-3">
                <div className="text-slate-500 uppercase font-bold border-b border-border pb-2">Simulation Control</div>
                <div className="flex flex-col gap-2">
                    <button
                        onClick={() => { setSystemState('HEALTHY'); setSelectedEventId(null); }}
                        className={`px-3 py-1.5 border ${systemState === 'HEALTHY' ? 'border-[#10B981] text-[#10B981] bg-[#10B981]/5' : 'border-slate-800 text-slate-500 hover:text-slate-300'}`}
                    >
                        [ Scenario 1: Healthy ]
                    </button>
                    <button
                        onClick={() => { setSystemState('OFFLINE'); setSelectedEventId(null); }}
                        className={`px-3 py-1.5 border ${systemState === 'OFFLINE' ? 'border-[#F59E0B] text-[#F59E0B] bg-[#F59E0B]/5' : 'border-slate-800 text-slate-500 hover:text-slate-300'}`}
                    >
                        [ Scenario 2: Offline ]
                    </button>
                    <button
                        onClick={() => { setSystemState('VIOLATION'); setSelectedEventId(null); }}
                        className={`px-3 py-1.5 border ${systemState === 'VIOLATION' ? 'border-[#EF4444] text-[#EF4444] bg-[#EF4444]/5' : 'border-slate-800 text-slate-500 hover:text-slate-300'}`}
                    >
            ...
            <header className="h-14 border-b border-border flex items-center justify-between px-6 shrink-0 z-50 bg-background">
                <div className="flex items-center gap-8">
                    <div className="text-slate-100 font-bold tracking-tighter text-xl">PAYFLUX</div>
                    <div className="flex items-center gap-2">
                        ...
                        <span className={`text-[10px] uppercase font-bold tracking-widest ${systemState === 'HEALTHY' ? 'text-slate-500' : systemState === 'OFFLINE' ? 'text-[#F59E0B]' : 'text-[#EF4444]'}`}>
                ...
                <div className="flex items-center gap-6">
                    ...
                    <button
                        disabled
                        className="px-3 py-1.5 border border-slate-800 bg-slate-900/50 text-slate-600 cursor-not-allowed uppercase font-mono text-[10px] tracking-widest"
            ...
            <main className="flex-1 flex overflow-hidden relative">
                ...
                <aside className="w-80 border-r border-border p-6 flex flex-col gap-8 bg-surface/30 shrink-0">
                    <section>
                        <h2 className="text-[10px] uppercase font-bold tracking-widest text-slate-500 mb-4 text-center">Telemetry Credibility</h2>
                        ...
                        {showTcsBreakdown && (
                            <div className="mt-6 border border-border bg-slate-900/50 p-4 mono text-[10px] space-y-3">
                                <div className="uppercase font-bold text-slate-500 border-b border-border pb-2">TCS Breakdown: {tcsValue} / 100</div>
                                <div className="space-y-1.5">
                                    <div className="flex justify-between items-center">
                                        <span className="text-slate-400">✓ Plaid: Active</span>
                                        <span className="text-[#10B981] font-bold">+28</span>
                                    </div>
                                    <div className="flex justify-between items-center">
                                        <span className={systemState === 'OFFLINE' ? 'text-[#EF4444]' : 'text-slate-400'}>
                                    ...
                                    <div className="flex justify-between items-center border-t border-border pt-2 mt-2">
                                        <span className="text-slate-100 uppercase">Total Score</span>
                    ...
                    <section>
                        <h2 className="text-[10px] uppercase font-bold tracking-widest text-slate-500 mb-4">Evidence Health</h2>
                        <div className="space-y-3 font-mono text-[11px]">
                            <div className="flex items-center justify-between">
                                <span className="text-slate-400 uppercase">Watermark Registry</span>
                                <span className={`${systemState === 'VIOLATION' ? 'text-[#EF4444]' : 'text-[#10B981]'} font-bold uppercase`}>
                                    {systemState === 'VIOLATION' ? 'Breached' : 'Stable'}
                                </span>
                            </div>
                            <div className="flex items-center justify-between">
                                <span className="text-slate-400 uppercase">Source Entropy</span>
                                <span className={`${systemState === 'VIOLATION' ? 'text-[#EF4444]' : 'text-[#10B981]'} font-bold uppercase`}>
                                    {systemState === 'VIOLATION' ? 'Unstable' : 'Nominal'}
                                </span>
                            </div>
                            <div className="flex items-center justify-between">
                                <span className="text-slate-400 uppercase">Processor Latency</span>
                ...
                <div className="flex-1 flex flex-col relative overflow-hidden">
                    <section className="flex-1 overflow-y-auto no-scrollbar">
                        <div className="sticky top-0 bg-background/90 backdrop-blur-md border-b border-border px-6 py-3 flex items-center justify-between z-20">
                            <h2 className="text-[10px] uppercase font-bold tracking-widest text-slate-500">Reserve History</h2>
                            <div className="flex gap-4">
                                <div className="flex items-center gap-1.5 font-mono text-[10px]">
                                    <div className={`w-1.5 h-1.5 rounded-full ${systemState === 'OFFLINE' ? 'bg-slate-700' : 'bg-[#10B981] animate-pulse'}`} />
                        ...
                        {systemState === 'OFFLINE' ? (
                            <div className="h-full flex flex-col items-center justify-center p-12 text-center">
                                ...
                                <h3 className="mono text-slate-100 font-bold mb-2">⚠ NO DATA RECEIVED FOR 18 MINUTES</h3>
                                <div className="space-y-4 max-w-md mx-auto">
                                    ...
                                    <div className="p-4 bg-[#F59E0B]/5 border border-[#F59E0B]/20 text-xs text-slate-300 leading-relaxed italic">
                            ...
                            <div className="divide-y divide-border/30 mono text-[11px]">
                                {currentEvents.map((item) => (
                                    <div
                                        ...
                                        className={`flex items-center gap-4 px-6 py-3 hover:bg-slate-900/50 cursor-pointer transition-colors group ${selectedEventId === item.id ? 'bg-slate-800/40 border-l-2 border-l-slate-300' : ''}`}
                                    >
                                        <div className={`severity-bullet ${item.color}`} />
                                        <span className="w-24 text-slate-500 group-hover:text-slate-300 transition-colors uppercase whitespace-nowrap">{item.time}</span>
                                        <span className={`w-16 font-bold ${item.sev === 'HIGH' ? 'text-[#EF4444]' : item.sev === 'MEDIUM' ? 'text-[#F59E0B]' : 'text-slate-500'}`}>{item.sev}</span>
                                        <span className="flex-1 text-slate-300 truncate">{item.event}</span>
                                        <div className="flex items-center gap-1.5 bg-background border border-border px-2 py-0.5">
                                            <span className="text-[9px] text-slate-600 font-bold uppercase tracking-tighter">TCS</span>
                    ...
                    <div className={`absolute bottom-0 left-0 right-0 border-t border-border bg-surface transition-transform duration-300 ease-in-out z-30 flex flex-col ${selectedEventId ? 'translate-y-0' : 'translate-y-[calc(100%-40px)]'}`}>
                        <div
                            ...
                            className="h-10 px-6 flex items-center justify-between cursor-pointer hover:bg-slate-800 transition-colors shrink-0"
                        >
                            <div className="flex items-center gap-3">
                                <span className="text-[10px] uppercase font-bold tracking-widest text-slate-500">Context Panel</span>
                                {selectedEvent && (
                                    <div className="flex items-center gap-2 mono text-[10px]">
                                        <span className="text-slate-600">|</span>
                                        <span className="text-slate-400 uppercase">{selectedEvent.time}</span>
                                        <span className="text-slate-600">—</span>
                                        <span className={`text-slate-200 ${systemState === 'VIOLATION' && selectedEvent.id === 99 ? 'text-[#EF4444] font-bold' : ''}`}>
                                            {selectedEvent.event}
                                        </span>
                                    </div>
                                )}
                            ...
                            {selectedEvent ? (
                                <div className="grid grid-cols-2 gap-8 h-full">
                                    ...
                                    <div className="space-y-6">
                                        <div>
                                            <h3 className="text-[10px] uppercase font-bold tracking-widest text-slate-500 mb-3">Event Metadata</h3>
                                            <pre className="bg-background border border-border p-4 mono text-[10px] text-slate-400 overflow-x-auto">
                                                {selectedEvent.details?.payload || 'No additional metadata available.'}
                                            </pre>
                                        </div>
                                        <div>
                                            <h3 className="text-[10px] uppercase font-bold tracking-widest text-slate-500 mb-2">Internal Recommendation</h3>
                                            <div className={`p-4 border-l-2 ${selectedEvent.tcs < 30 ? 'border-l-[#EF4444] bg-[#EF4444]/5' : 'border-l-[#F59E0B] bg-[#F59E0B]/5'} text-xs text-slate-200 leading-relaxed`}>
                                    ...
                                    <div>
                                        <h3 className="text-[10px] uppercase font-bold tracking-widest text-slate-500 mb-3">Event Credibility: {selectedEvent.tcs} / 100</h3>
                                            ...
                                            {selectedEvent.details?.breakdown ? (
                                                selectedEvent.details.breakdown.map((b, i) => (
                                                    <div key={i} className="px-4 py-2.5 flex justify-between items-center font-bold">
                                                        <span className={b.type === 'plus' ? 'text-slate-300' : 'text-[#EF4444]'}>
                                                            {b.type === 'plus' ? '✓' : '⚠'} {b.label}
                                                        </span>
                                                ...
                                                <div className="p-4 text-slate-500 italic">No breakdown available for this score.</div>
                                            )}
                                            <div className="px-4 py-3 bg-slate-900/50 flex justify-between items-center text-slate-100">
